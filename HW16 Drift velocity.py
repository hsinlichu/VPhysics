...
while True:
    ...
    if outside[N-1,0] or outside[L-1,1] or outside[N-1,2]:atoms[N-1].retain=0
    pos_array[outside]=- pos_array[outside]
    vv+=vector(sum(v_array,0)/N) ##from index 0 add v_array

    for i in range(N):
        test=random.rand(1,1).flatten ## let the array to be only element
        if test<=0.0008:
            counter+=1
